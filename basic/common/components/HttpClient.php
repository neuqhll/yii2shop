...
class HttpClient  extends  BaseService{
    ...
    {
        $calculate_time1 = microtime(true);//返回浮点类型的Unix 时间戳的微秒数
        // 初始化
        $curl = curl_init();  //初始化,返回$curl句柄
        //url_setopt — 设置 cURL 传输选项
        ...
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);//FALSE禁止 cURL 验证对等证书（peer's certificate）。要验证的交换证书可以在 CURLOPT_CAINFO 选项中设置，或在 CURLOPT_CAPATH中设置证书目录。
        //curl_setopt($curl, CURLOPT_VERBOSE, true); //打印日志

        curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1);//函数中加入下面这条语句,TRUE 时将会根据服务器返回 HTTP 头中的 "Location: " 重定向
